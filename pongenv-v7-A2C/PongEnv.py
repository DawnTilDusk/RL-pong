#env-v2 内置了get_state()方法，用于获取当前状态
import pygame
import numpy as np 
...
class PongEnv:
    ...
    def get_state(self):
        ...
        ball_dx = self.ball_speed[0] / self.ball_maxspeed
        ball_dy = self.ball_speed[1] / self.ball_maxspeed
        l_dy = self.pad_speed[0] / self.pad_maxspeed
        r_dy = self.pad_speed[1] / self.pad_maxspeed
        # positions normalized to [-1, 1]
        ball_x = (self.ball_x / self.WINDOW_WIDTH) * 2 - 1
        ball_y = (self.ball_y / self.WINDOW_HEIGHT) * 2 - 1
        l_y = (self.pad_y[0] / self.WINDOW_HEIGHT) * 2 - 1
        r_y = (self.pad_y[1] / self.WINDOW_HEIGHT) * 2 - 1
