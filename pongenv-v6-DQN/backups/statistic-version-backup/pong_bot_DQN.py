...
class Bot_DQN:
    def __init__(self, env: PongEnv, side = 1, alpha = 1e-4, gamma = 0.99, epsilon = 0.3, decaying = 0.999995, epsilon_min = 0.001, stats_stride: int = 10):
        self.env = env
        self.side = side
    ...
    def plot_training_curves(self, save_path=None, show=True):
        ...
        if len(self.q_values_history) > 0:
            ...
            try:
                ...
                for t in ticks:
                    idx = int(round(t))
                    if idx <= 0:
                        labels.append('0' if idx == 0 else '')
                    else:
                        labels.append(str(int(idx * max(self.stats_stride, 1))))
        ...
        if len(self.epsilon_history) > 0:
            ...
            try:
                ax2_top.set_xticks(ticks2)
                labels2 = []
                for t in ticks2:
                    idx2 = int(round(t))
                    if idx2 <= 0:
                        labels2.append('0' if idx2 == 0 else '')
                    else:
                        labels2.append(str(int(idx2 * max(self.stats_stride, 1))))
                ax2_top.set_xticklabels(labels2)
            except Exception:
                pass
        ...
        # 子图3：近期胜率
        plt.subplot(2,2,3)
        plt.plot(self.win_rate_history, label='win rate(100)')
        plt.title(titles[2])
        plt.xlabel('episode')
        plt.ylabel('win rate')
        plt.grid(True)
        # 子图4：近期平均奖励
        plt.subplot(2,2,4)
        plt.plot(self.avg_reward_history, label='avg reward(10)')
        plt.title(titles[3])
        plt.xlabel('episode')
    ...
    def save_backup_package(self, show_plot=False):
        ...
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        # 集中备份目录（绝对路径）
        base_dir = r"C:\Users\Notebook\Desktop\work\project_dev\pong\pongenv-v6(DQN)\history"
        try:
